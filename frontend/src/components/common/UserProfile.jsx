...
import { Email, Phone, Badge, Work } from '@mui/icons-material';
    ...
    >
      ...
      <Box sx={{ p: 2, minWidth: 280 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
          <Avatar
            src={rostoFuncionario}
            alt={user?.nome || 'Funcionário'}
            sx={{ width: 48, height: 48, bgcolor: '#f59e0b', fontSize: '1.2rem' }}
          />
          <Box sx={{ flex: 1 }}>
            ...
            {user.cpf && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  CPF: {String(user.cpf).replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')}
                </Typography>
              </Box>
            )}
            {user.telefone && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  {String(user.telefone).replace(/(\d{2})(\d{5})(\d{4})/, '($1) $2-$3')}
                </Typography>
              </Box>
            )}
            {user.matricula && (
